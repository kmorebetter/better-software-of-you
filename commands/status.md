...
Query the database at `${CLAUDE_PLUGIN_ROOT}/data/soy.db` and present a clear status overview.
...
Read module manifests from `${CLAUDE_PLUGIN_ROOT}/modules/*/manifest.json` to identify active cross-module enhancements (where both modules in an enhancement pair are installed).
