...
- **Enrich contacts from interactions.** The most valuable thing on a contact is its **interaction-derived relationship context**, not firmographics. Whenever a new transcript is ingested/analyzed or a new email thread is synced, refresh the `notes` context of every contact linked to it — WHO they are, the RELATIONSHIP to the user, what was DISCUSSED, and the current STATE / open thread — grounded in that interaction and dated. Ground every claim (see "Data Integrity: Never Fabricate"); the richest sources are Granola transcripts and Gmail, not the local `emails` table. New people named in a transcript who aren't yet contacts are candidates to card (with a transcript-grounded role + context); never guess a role — leave it NULL if unstated. After refreshing context, rebuild the visual contact sheets so they stay current — run `python3 "${CLAUDE_PLUGIN_ROOT:-$(pwd)}/scripts/build_contact_pages.py"` (the `/contact-pages` command, a read-only batch renderer). Write each contact's context into `notes` using `WHO:` / `RELATIONSHIP:` / `DISCUSSED:` / `STATE:` (or `OPEN THREAD:`) / `Basis:` labels so the brief renders as a structured visual, not a wall of text.
